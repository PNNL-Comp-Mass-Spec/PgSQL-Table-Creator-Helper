...
namespace PgSqlTableCreatorHelper
{
    internal class Program
    {
        ...
        private static int Main(string[] args)
        {
            ...
            var version = TableCreatorHelperOptions.GetAppVersion();

            var parser = new CommandLineParser<TableCreatorHelperOptions>(asmName.Name, version)
            {
                ProgramInfo = "This program processes a SQL DDL file with `CREATE INDEX` commands and renames the " +
            ...
            try
            {
                var processor = new TableCreatorHelper(options);
